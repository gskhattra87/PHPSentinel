...
class MalwareAnalyzer:
    ...
    """
    def __init__(self):
        # Initializing CodeBERT as the foundation for code quantification
        ...
        # Head A (Malicious/Benign) + Head B (Multi-Label Behaviors)
        # num_labels=6: [0:Benign, 1:RCE, 2:Exfiltration, 3:Shell, 4:Persistence, 5:File-Ops]
        """ self.model = AutoModelForSequenceClassification.from_pretrained(
            "microsoft/codebert-base", num_labels=6
        ) 
    ...
    async def analyze_code(self, raw_code: str):
        ...
        current_risk_profile = {
            "RCE": probs[0][1].item(),
            "Exfiltration": probs[0][2].item(),
            "ReverseShell": probs[0][3].item(),
            "Persistence": probs[0][4].item(),
            "FileManipulation": probs[0][5].item()
        }
        
        # 4. Selective Prediction & Decision Engine (RQ5)
        decision_data = selective_prediction(confidence, current_risk_profile)
        ...
        return {
            "malicious_score": sum(current_risk_profile.values()),
    ...
    async def get_semantic_evidence(self, code: str, risk_profile: dict):
        ...
        patterns = {
            "RCE": [r"eval\(", r"assert\(", r"base64_decode", r"shell_exec"],
